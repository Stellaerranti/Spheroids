...
def simulate_model(params, empirical_days, N_grid=81, L=3.0, dt=0.002):
    """
    Integrate the PDE with internal time step ``dt`` and save the state at
    the requested experimental-time coordinates in ``empirical_days``.

    Despite the historical argument name, ``empirical_days`` may contain
    arbitrary output times (for example 0.0, 0.1, 0.2, ...).  The fitting
    code can therefore continue to request only the measured days, while a
    final simulation can request a much denser time grid.
    """
    (
        lambda_,
        delta,
    ...
    output_days = np.asarray(empirical_days, dtype=float)
    if output_days.size == 0:
        return {}
    if np.any(output_days < 0):
        raise ValueError("Output days must be non-negative.")

    output_model_times = output_days * time_scale

    T_max = np.max(output_model_times)
    steps = int(np.ceil(T_max / dt))

    # Several requested days can occasionally round to the same PDE step,
    # especially for very small output intervals.  Store a list instead of
    # overwriting one requested day with another.
    output_steps = {}
    for model_time, day in zip(output_model_times, output_days):
        step_index = int(round(model_time / dt))
        output_steps.setdefault(step_index, []).append(float(day))

    results = {}

    for t_step in range(steps + 1):
        if t_step in output_steps:
            for day in output_steps[t_step]:
                results[day] = {
                    "H": h.copy(),
                    "N": n.copy(),
                    "dx": dx,
...
def make_subgrid_field(H, dx, factor=10):
    """
    Interpolate H onto a finer spatial grid for measurement only.
    The PDE itself is still solved on the original grid.
    """

    H_fine = zoom(H, factor, order=1)

    # Because there are factor times more pixels per model distance
    dx_fine = dx / factor

    return H_fine, dx_fine

def calculate_model_observables(
    H,
    dx,
    pixel_scale=1.0,
    ...
    H_measure, dx_measure = make_subgrid_field(
        H,
        dx,
        factor=subgrid_factor,
    )

    mask_area = bf.get_spheroid_mask(
        H_measure,
        threshold=threshold,
    )
    ...
    # Geometry measured on interpolated field
    area_model = bf.area_from_mask(
        mask_area,
        dx=dx_measure,
    )

    perimeter_model = bf.perimeter_from_mask(
        mask_area,
        dx=dx_measure,
    )

    feret_model = bf.feret_diameter_from_mask(
        mask_area,
        dx=dx_measure,
    )
    ...
    # Intensity measured on same interpolated grid
    H_scaled = H_measure * intensity_scale
    values = H_scaled[mask_area]

    gray_mean = np.mean(values)

    # Convert to empirical image units
    ...
    # Keep integral on original PDE grid
    total_h = (
        np.sum(H)
        * dx**2
        * pixel_scale**2
...
def make_dense_output_days(empirical_days, output_interval=0.1, start_day=0.0):
    """
    Build a dense experimental-time grid and explicitly include every
    empirical observation day.

    Example with output_interval=0.1:
        0.0, 0.1, 0.2, ..., 9.9, 10.0

    If an empirical observation lies off that regular grid, it is added too.
    """
    empirical_days = np.asarray(empirical_days, dtype=float)

    if empirical_days.size == 0:
        raise ValueError("empirical_days is empty")
    if output_interval <= 0:
        raise ValueError("output_interval must be positive")

    end_day = float(np.max(empirical_days))
    start_day = float(start_day)

    dense_days = np.arange(
        start_day,
        end_day + 0.5 * output_interval,
        output_interval,
        dtype=float,
    )

    # Floating-point rounding keeps dictionary keys / CSV output clean.
    dense_days = np.round(dense_days, 10)
    empirical_days = np.round(empirical_days, 10)

    return np.unique(np.concatenate([dense_days, empirical_days]))


def evaluate_dense_simulation(
    params,
    empirical_data,
    output_interval=0.1,
    start_day=0.0,
    ...
    """
    Evaluate the fitted model on a dense time grid, independently of the
    sparse empirical sampling used by the objective function.

    Returns one row per requested day with all model observables.
    """
    (
        lambda_,
        delta,
    ...
    output_days = make_dense_output_days(
        empirical_data["Day"].values,
    ...
    for day in output_days:
        ...
        if day not in sim_results:
            raise RuntimeError(f"Requested day {day} was not saved by simulation")
        ...
        rows.append({
            "Day": day,
            "ModelRadius": obs["Radius"],
            "ModelArea": obs["Area"],
            "ModelGrayMean": obs["GrayMean"],
            "ModelIntDen": obs["IntDen"],
            "ModelCircularity": obs["Circularity"],
            "ModelFeret": obs["Feret"],
            "ModelPerimeter": obs["Perimeter"],
...
def save_dense_fit_plots(empirical_data, dense_table, out_dir="fit_output_dense"):
    """
    Plot empirical measurements only at measured days and draw the model as
    a dense continuous-looking trajectory between them.
    """
    ...
    for target, ylabel in plot_specs:
        ...
        if mean_col not in empirical_data.columns or model_col not in dense_table.columns:
            continue
        ...
        if std_col in empirical_data.columns:
            plt.errorbar(
                empirical_data["Day"],
                empirical_data[mean_col],
                yerr=empirical_data[std_col],
                fmt="o",
                capsize=3,
                label="Empirical",
            )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(out_dir, f"{target}_fit_dense.png"),
            dpi=300,
        )
        plt.close()

    if "ModelTotalH" in dense_table.columns:
        plt.figure(figsize=(6, 4))
        plt.plot(
            dense_table["Day"],
            dense_table["ModelTotalH"],
            "-",
            label="Model TotalH",
        )
        plt.xlabel("Day")
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(out_dir, "TotalH_diagnostic_dense.png"),
            dpi=300,
        )
        plt.close()


def save_dense_simulation(
    params,
    empirical_data,
    out_dir="fit_output_dense",
    output_interval=0.1,
    start_day=0.0,
    N_grid=81,
    L=3.0,
    ...
    """Run, save and plot the dense post-fit simulation."""
    os.makedirs(out_dir, exist_ok=True)

    dense_table = evaluate_dense_simulation(
        params=params,
    ...
    dense_table.to_csv(
        os.path.join(out_dir, "dense_simulation.csv"),
        index=False,
    )

    save_dense_fit_plots(
        empirical_data=empirical_data,
...
if __name__ == "__main__":
    ...
    best_json_path=(
        "seed_runs_mu_2000_int/"
        "best_seed_summary.json"
    ),
    empirical_data=empirical_data,
    targets=targets,
    bounds=bounds,
    out_dir="local_fit_mu_2000_int",
    N_grid=N_GRID,
    L=L,
    dt=DT,
    threshold=H_THRESHOLD,
    ...
    # Dense post-fit trajectory.  This does NOT change the fitting objective;
    # it only samples the final fitted PDE solution more frequently.
    dense_fit_table = save_dense_simulation(
        params=local_result.x,
        empirical_data=empirical_data,
        out_dir="local_fit_mu_2000_int/dense_simulation",
    ...
    print("\nDense simulation preview:")
    print(dense_fit_table.head(12))
